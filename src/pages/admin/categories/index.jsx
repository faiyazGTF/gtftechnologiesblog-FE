...
const BlogCategories = () => {
  ...
  const API_ASSETS = process.env.NEXT_PUBLIC_API_ASSETS_URL;

  const fetchBlogs = async (page = 1) => {
    try {
      const res = await axiosAdmin.get(
        `${API_ADMIN_URL}blog-category?page=${page}&limit=${limit}`
      );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await axiosAdmin.delete(
        `${API_ADMIN_URL}blog-category/${id}/delete`
      );
      if (res.data.status) {
                    ...
                    {categories.length > 0 ? (
                      categories.map((blog) => (
                        <TableRow key={blog._id}>
                          <TableData heading={blog.name && blog.name.length > 110 ? blog.name : undefined}>
                            {blog.name && blog.name.length > 110
                              ? `${blog.name.slice(0, 110)}...`
                              : blog.name}
                          </TableData>
                          ...
                          <TableData>
                            <div className="w-full flex items-center justify-center gap-[10px]">
                              <Link
                                href={`/admin/categories/edit/?id=${blog.id}`}
